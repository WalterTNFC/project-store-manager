...
describe('Testa a camada service de products', () => {
  ...
  //   before(async () => {
  //     sinon.stub(productsModel, 'updateProduct').returns({
  //       id: 1,
  //       name: 'Nome #01',
  //     });
  //   });

  //   after(async () => productsModel.updateProduct.restore());

  //   it('Verifica se retorna um objeto', async () => {
  //     const response = await productsService.updateProduct('Nome #01');
  //     expect(response).to.be.an('object');
  //   });
  ...
  describe('Testa a atualização de um produto', () => {
    ...
    //     const response = await productsService.newProducts(1, 'Produto #01');
    //     expect(response).to.be.an('object');
    //     expect(response).to.have.key('error');
    //   });
